...
TEST(Line2D, distanceTo)
{
  ...
  ASSERT_NEAR( 1.0, l.distanceTo(tf2::Point2D(1,0)), tolerance);
  ASSERT_NEAR( 0.0, l.distanceTo(tf2::Point2D(2,0)), tolerance);
  ASSERT_NEAR(-1.0, l.distanceTo(tf2::Point2D(3,0)), tolerance);
}

TEST(Line2D, pointOnLine)
{
  tf2::Line2D l(2.0, -2.0, 2.0, 2.0);
  ASSERT_TRUE( tf2::Point2D(2,0) == l.pointOnLine(tf2::Point2D(1,0)));
}