...
class LineSegment2D : public Line2D {
  ...
  public:
    ...
    /**
     * computes the squared distance to line segment
     * @see http://stackoverflow.com/questions/849211/shortest-distance-between-a-point-and-a-line-segment
     * @param p point
     * @param dx vector to point x
     * @param dx vector to point y
     * @return distance to line between the segment endpoints or the distance to the nearest endpoints
     **/
    tf2Scalar distance_to_sqrt(const Point2D &p, tf2Scalar &dx, tf2Scalar &dy) const;
     ...
     **/
    Point2D closest_point_to(const Point2D &p) const;
    ...
    private:
       ...
       * helper function 
      **/
    tf2Scalar closest_point_to_ratio(const Point2D &p) const;
};
