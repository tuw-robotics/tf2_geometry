...
TEST(pose2d, constructor)
{
  ...
  ASSERT_NEAR(M_PI/4, pose0.theta(), tolerance);
}
...
TEST(pose2d, transform_into_base)
{
  ...
  // Pose
  tf2::Pose2D pose_in_target(0.0, sqrt(2)*2, -M_PI/2.0);
  tf2::Pose2D pose_in_base = pose0.transform_into_base(pose_in_target);
  ASSERT_NEAR(1, pose_in_base.x(), tolerance);
  ASSERT_NEAR(1, pose_in_base.y(), tolerance);
  ASSERT_NEAR(-M_PI/4, pose_in_base.theta(), tolerance);
}
